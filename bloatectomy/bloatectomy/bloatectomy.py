...
class bloatectomy():
    ...
    def tokenize2(regex, token_in):
        ...
        for token in sent_token:
            # Handle multiple consecutive newlines - collapse to single newline
            temp_token = re.sub(r"\n{2,}", "\n", token)
            ...
            # Clean up excess spaces around newlines but preserve the newlines
            temp_token = re.sub(r" +\n +", "\n", temp_token)
            temp_token = re.sub(r" +\n", "\n", temp_token)
            temp_token = re.sub(r"\n +", "\n", temp_token)
